...
| ⸱ **[dedot](../../skills/depth/dedot/SKILL.md)** | Предлагает запятую или связующее слово с обоснованием для каждой средней точки, соединяющей открытое перечисление в корейской прозе; сохраняет три допустимых употребления и защищённые контексты | корейская проза в границах, заданных автором | user | ✔ |
...
- `readback` переформулирует запрос и спрашивает только тогда, когда выживает настоящая развилка.
