...
5. **Complete a new skill, don't merge it raw.** Get the name right (a plain real word or tight compression, no opaque coinage), the invocation right (model- vs user-invoked), and the home right; then register it on every roster surface (`plugin.json`, the README Index in root and every localized copy, `re0-upgrade`'s Current catalog, `scripts/runtime/catalog.cjs`) so no drift-guard trips.
