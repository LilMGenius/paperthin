...
4. **Approve as you accept, then land on a `land/pr-<n>` branch** (a range only for a genuine multi-PR batch). Submit approval when you decide to accept, before landing. This verdict on the contributor's code records a closed-not-merged PR as accepted without waiting for release or closure. Preserve each contributor commit's authorship, becoming its committer, and clean its message with `re0-git`. Make each maintainer change a separate commit to keep credit legible, then fast-forward into `main`.
...
- Any reviewer runs the installed pipeline skills (`shower`, `re0-git`, `re0-release`) and performs missing skills' steps by hand.
