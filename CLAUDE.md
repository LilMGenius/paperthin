...
5. **[package.json](./package.json)** bumps version (a new skill or a new capability on an existing one = minor; fix/docs = patch; a skill removed with no replacement path = major); `keywords` stay grouped logically.
