...
2. **[README](./README.md)** and its localized copies under [`docs/readme/`](./docs/readme/) list it — perspective group, invocation column, linked; the root README is the English source, and every translation keeps the `<sub>Read in: …</sub>` switcher with links rebased from its own directory.
