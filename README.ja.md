...
# Paperthin: 低レベルのエージェント設計パターン

<img src="https://raw.githubusercontent.com/LilMGenius/paperthin/main/assets/banner.svg" alt="Paperthin - 作者ではなく成果物を信じる。" width="820">

**昔からあるエンジニアリングの知恵を、エージェントが自分で取りに行く反射動作に変える。**

**どの**エージェントでも | Claude Code、Codex、OpenCode、Antigravity、Copilot、Cursor、Grok-Build、Pi、Hermes、OpenClaw など。
...
1. 使っているすべてのエージェントに**インストール**します。
   ```bash
   npx skills@latest add LilMGenius/paperthin --global --agent '*'
   ```
2. skills がコピーではなく symlink されるように、昇格権限で実行します。そうすると自動更新されます。
3. **使います**。モデルが呼び出すので、必要なときにエージェントが自分で取りに行きます。`/re0` のように名前で直接呼んでもかまいません。

**迷ったら？** 使っているエージェントに上のコマンドを貼り付けて、`set this up for me` と言ってください。残りはやってくれます。

<a id="the-map"></a>
## マップ

**成果物はいくつあり、どれだけの時間にまたがるのか？**

<div align="center">
<img src="https://raw.githubusercontent.com/LilMGenius/paperthin/main/assets/map.svg" alt="LilMGenius/paperthin の Paperthin マップ。2x2 の行列。横軸は個数（一つ、その後に多数）、縦軸は時間（今、その後に反復をまたぐ）。四つの領域。左上の depth: 一つの成果物、今。これはきれいで真か？右上の breadth: 多数の成果物、今。一つの真実がどこでも一貫しているか？左下の coil: 一つのプロジェクト、反復をまたぐ。各パスは次を教えたか？右下の mesh: 多数の思考、複数ラウンド。群れは真実へ収束するか？" width="820">
</div>
...
| Skill | 何をするか | 範囲 | 呼び出し元 |
|---|---|---|---|
| ♻️ **[re0](./skills/depth/re0/SKILL.md)** | drift した成果物を、さらにパッチするのではなく、きれいな v0 として書き直す | 成果物一つ | モデル |
| 🚿 **[shower](./skills/depth/shower/SKILL.md)** | 新鮮でゼロコンテキストの目で冷たく読む。これ単体で成立するか？ *(読み取り専用)* | 成果物一つ | モデル |
| 🔬 **[factchk](./skills/depth/factchk/SKILL.md)** | 主張をソースに対して両方向に検証する。ばかげたことが本当で、当然に見えることが偽かもしれないか？ *(読み取り専用 → 修正)* | 主張一つ | モデル |
| 🧪 **[mandela](./skills/depth/mandela/SKILL.md)** | validation の leakage を監査する。外部の ground truth は実際に入っているか？ *(読み取り専用)* | eval 一つ | モデル |
| 🛣️ **[autobahn](./skills/depth/autobahn/SKILL.md)** | 危険な scope を先に切り出し、安全な残りを全力で走らせ、descope ledger を出す | タスク一つ | モデル |
| 🥄 **[sip](./skills/depth/sip/SKILL.md)** | 変更後、自分の出力をリポジトリ自身の clean-and-true checks で味見する | 自分の出力 | モデル |
...
| Skill | 何をするか | 範囲 | 呼び出し元 |
|---|---|---|---|
| 🔎 **[ssotchk](./skills/breadth/ssotchk/SKILL.md)** | 一つの事実が散らばったり重複したりしている場所を見つけ、canonical source を名指しする *(読み取り専用)* | 事実一つ、多数の場所 | モデル |
| 🧲 **[ssotize](./skills/breadth/ssotize/SKILL.md)** | それを一つの家に統合し、残りをそこへ向ける | 事実一つ、多数の場所 | モデル |
...
| Skill | 何をするか | 範囲 | 呼び出し元 |
|---|---|---|---|
| 🧭 **[retro](./skills/coil/retro/SKILL.md)** | 完了または失敗した cycle から教訓と anti-pattern を抽出する | 完了した cycle 一つ | モデル |
| 🧱 **[re0-work](./skills/coil/re0-work/SKILL.md)** | 再利用に値した教訓だけを残して v0 からやり直す | やり直し一つ | モデル |
| 🌀 **[flywheel](./skills/coil/flywheel/SKILL.md)** | build → QA → retro → re0-work ループを回し、コードではなく学習を複利化する | ループ全体 | モデル |
...
**ほとんどのエージェント skill は slop です。**

エージェントに目標を渡すと、エージェントは**足します**。ファイルを増やし、選択肢を増やし、「役に立つ」ボイラープレートを増やします。足すことは進捗に見え、何も戻って削除させません。
...
- `ssotchk` / `ssotize` はファイル間に散らばった同じ事実を畳みます。
...
**修正 → `re0`:** 成果物を最初の版のように、きれいな v0 として書き直します。
...
**修正 → `shower`:** あなたのセッションを見たことのない見知らぬ人に成果物だけを渡し、「これは本当に意味が通るか？」と聞きます。
...
**修正 → `ssotchk` + `ssotize`:** 散らばりを見つけ、canonical source を名付け、統合して残りをそこへ向けます。

> *先行例: [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself) - 一つの事実、一つの authoritative home (Hunt & Thomas, The Pragmatic Programmer, 1999)。*

### #4 - 「検証を忘れるな」は発火しない
docs に埋めた guideline は、真新しい session では発火しません。まさに author bias が最も高い瞬間です。

**修正 → `sip`:** 何かを終えた瞬間、clean checks (`shower`, `ssotchk`, `re0`) を走らせ、claim や eval があれば true checks (`factchk`, `mandela`) も自動で出力に走らせます。
...
- **Result** - dogfooding で二つの fault が出て、どちらも修正されました。
...
**修正 → `factchk`:** 現実に根ざした claim は、ship 前に外部 source と両方向で検証します。source に届かないときは推測せず、flag します。
...
### #7 - eval は自分自身を確認する
model、scorer、designer が全員、結果は本物だと同意できます。しかし外部 ground truth はループに一度も入っていないかもしれません。部屋全体が、独立して起きていないことを自信を持って覚えている状態です。
...
- **Result** - その一つの project で leakage は八つの形で現れました。自分が描いた bucket を scorer が採点する、二つの component が shared space で互いを「検証」する、private recipe が verifier を designer にする。これが skill の 8-pattern taxonomy になりました。
...
- **Setup** - ある game-engine demo は full-stack で runnable な状態に到達しました。API routes、canvas runtime、leaderboard、arcade pages、remix と telemetry panels、tests、screenshots。
